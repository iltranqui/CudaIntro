...
ConvolutionalNeuralNetwork::ConvolutionalNeuralNetwork() {
    ...
    // Update flattened size to use the output of conv4 layer
    flattened_size = CONV4_FILTERS * conv4_output_height * conv4_output_width;
...
void ConvolutionalNeuralNetwork::initializeWeights() {
    ...
    h_fc3_weights.resize(FC2_SIZE * OUTPUT_SIZE);
    for (auto& w : h_fc3_weights) {
        w = fc3_dist(gen);
    }

    // FC3 bias
    h_fc3_bias.resize(OUTPUT_SIZE, 0.01f);
