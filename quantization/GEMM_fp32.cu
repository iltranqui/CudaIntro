...
__global__ void fill_random_fp32(float* data, int64_t count, uint64_t seed,
                                 float lo, float hi) {
...
GemmResult run_gemm_fp32(cublasHandle_t handle) {
  ...
  CHECK_CUDA(cudaMalloc(&a, a_elems * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&b, b_elems * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&c, c_elems * sizeof(float)));

  constexpr int threads = 256;
  int blocks_a = static_cast<int>((a_elems + threads - 1) / threads);
  int blocks_b = static_cast<int>((b_elems + threads - 1) / threads);
  fill_random_fp32<<<blocks_a, threads>>>(a, a_elems, 0x3234U, kMin, kMax);
  fill_random_fp32<<<blocks_b, threads>>>(b, b_elems, 0x7776U, kMin, kMax);
  ...
  CHECK_CUBLAS(cublasGemmEx(
      ...
      static_cast<int>(N), static_cast<int>(K), &alpha, a, CUDA_R_32F,
      static_cast<int>(M), b, CUDA_R_32F, static_cast<int>(K), &beta, c,
      CUDA_R_32F, static_cast<int>(M), CUBLAS_COMPUTE_32F,
