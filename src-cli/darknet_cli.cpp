...
extern void run_precision_benchmark();
...
int main(int argc, char **argv)
{
	...
	try
	{
		// Install universal fatal-error handlers and open exception.log before
		// anything else.  The signal()s below then override the signals Darknet
		// already handles (SIGSEGV/SIGFPE/SIGILL/SIGABRT), so install_fatal_handlers
		// only retains SIGBUS, std::terminate (unhandled exceptions), and the
		// watchdog.  Watchdog disabled (0) — no heartbeat() calls wired up yet.
		Darknet::install_fatal_handlers(0);
		...
		else if (cfg_and_state.command == "precisionbenchmark") { run_precision_benchmark(); }
