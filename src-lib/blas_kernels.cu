...
__global__ void sgd_update_kernel(int N, float *values, float *updates, float rate, float momentum, float decay)
{
	int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (i < N) {
		const float update = updates[i] + decay * values[i];
		values[i] += rate * update;
		updates[i] = momentum * update;
	}
}
...
void sgd_update_ongpu(int N, float *values, float *updates, float rate, float momentum, float decay)
{
	TAT(TATPARMS);

	if (N <= 0) return;
	sgd_update_kernel<<<cuda_gridsize(N), BLOCK, 0, get_cuda_stream()>>>(N, values, updates, rate, momentum, decay);
	CHECK_CUDA(cudaPeekAtLastError());
}
...
void simple_copy_ongpu(int size, float *src, float *dst)
{
	...
	const size_t size_bytes = static_cast<size_t>(size) * sizeof(float);
	CHECK_CUDA(cudaMemcpyAsync(dst, src, size_bytes, cudaMemcpyDeviceToDevice, get_cuda_stream()));
	CHECK_CUDA(cudaPeekAtLastError());
}
...
void fill_ongpu(int N, float ALPHA, float * X, int INCX)
{
	...
	if (N <= 0) return;
	if (ALPHA == 0.0f && INCX == 1) {
		const size_t size_bytes = static_cast<size_t>(N) * sizeof(float);
		CHECK_CUDA(cudaMemsetAsync(X, 0, size_bytes, get_cuda_stream()));
		CHECK_CUDA(cudaPeekAtLastError());
		return;
	}
