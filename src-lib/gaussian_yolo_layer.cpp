	...
	*(l.cost) = 0;
	...
	for (int b = 0; b < l.batch; ++b)
	{
		for (int j = 0; j < l.h; ++j)
		{
			for (int i = 0; i < l.w; ++i)
			{
				for (int n = 0; n < l.n; ++n)
				{
					...
					const Darknet::Box pred_metric = pred;
		...
		for (int t = 0; t < l.max_boxes; ++t)
		{
			...
			float best_iou = 0;
			int best_n = 0;
			const Darknet::Box model_truth = truth;
			int i = std::max(0, std::min(l.w - 1, static_cast<int>(model_truth.x * l.w)));
			int j = std::max(0, std::min(l.h - 1, static_cast<int>(model_truth.y * l.h)));
			...
			{
				i = std::max(0, std::min(l.w - 1, static_cast<int>((model_truth.x - model_truth.w / 2.0f) * l.w)));
				j = std::max(0, std::min(l.h - 1, static_cast<int>((model_truth.y - model_truth.h / 2.0f) * l.h)));
			}
			...
			Darknet::Box truth_shift = model_truth;
			...
			if(mask_n2 >= 0)
			{
				...
				const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
				float iou = delta_gaussian_yolo_box(model_truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w, state.net.h, l.delta, (2-truth.w*truth.h), l.w*l.h, l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1, l.yolo_point, l.max_delta);
