...
void init_cublas();  // Initialize cuBLAS early before CUDNN allocations
...
/** Route all CUDA work from the calling thread to @p net's dedicated stream, cuBLAS handle,
 * and (if built with cuDNN) cuDNN handle.  Call this once at the start of any thread that will
 * invoke @c network_predict on @p net.  Because the overrides are thread-local, concurrent
 * prediction threads each pointing to different networks never interfere with each other, and
 * every kernel/BLAS call in the thread automatically routes to @p net's private stream without
 * any per-call changes.
 */
void activate_network_streams(Darknet::Network & net);

/** Revert the calling thread's stream/handle overrides back to the global device-indexed
 * singletons.  Optional for threads that own a single network for their whole lifetime.
 * Useful when a thread needs to switch networks mid-life or return to the global stream.
 */
