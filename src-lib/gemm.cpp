...
namespace
{
	// TF32 tensor-core math for all FP32 GEMMs, enabled by "-tf32" (net.tf32)
	bool tf32_gemm_enabled = false;
}

void gemm_set_tf32(bool enable)
{
	tf32_gemm_enabled = enable;
}
		...
		float *C_gpu, int ldc)
{
	...
	cublasStatus_t stream_status = cublasSetStream(handle, get_cuda_stream());
	CHECK_CUBLAS(stream_status);
#if defined(DARKNET_GPU_CUDA) && defined(CUBLAS_VERSION) && CUBLAS_VERSION >= 11000
	if (tf32_gemm_enabled)
	{
...
void gemm_ongpu_strided_batched(int TA, int TB, int M, int N, int K, float ALPHA,
		float *A_gpu, int lda, long long strideA,
		float *B_gpu, int ldb, long long strideB,
		float BETA,
		float *C_gpu, int ldc, long long strideC,
		int batch_count)
{
	TAT(TATPARMS);
	...
	if (tf32_gemm_enabled)
	{
		cublasStatus_t status = cublasGemmStridedBatchedEx(handle,
				...
				N, M, K,
				&ALPHA,
				B_gpu, CUDA_R_32F, ldb, strideB,
				A_gpu, CUDA_R_32F, lda, strideA,
				&BETA,
				C_gpu, CUDA_R_32F, ldc, strideC,
				batch_count,
				CUBLAS_COMPUTE_32F_FAST_TF32,
				CUBLAS_GEMM_DEFAULT_TENSOR_OP);
		if (status == CUBLAS_STATUS_SUCCESS)
...
void gemm_ongpu_tensor_op(int TA, int TB, int M, int N, int K, float ALPHA,
		float *A_gpu, int lda,
		float *B_gpu, int ldb,
		float BETA,
		float *C_gpu, int ldc,
...
void gemm_ongpu_strided_batched_tensor_op(int TA, int TB, int M, int N, int K, float ALPHA,
		float *A_gpu, int lda, long long strideA,
		float *B_gpu, int ldb, long long strideB,
		float BETA,
		float *C_gpu, int ldc, long long strideC,
		...
		bool use_tensor_op)
{
	...
	gemm_ongpu_strided_batched(TA, TB, M, N, K, ALPHA,
		A_gpu, lda, strideA, B_gpu, ldb, strideB, BETA,
		C_gpu, ldc, strideC, batch_count);
}
