...
namespace
{
	...
	static constexpr uint32_t k_fixed_weight_init_seed = 0u;
	static bool use_random_weight_init_seed = true;
	static std::mt19937 weight_init_engine(k_fixed_weight_init_seed);
	...
	static inline std::mt19937 & get_weight_init_engine()
	{
		TAT(TATPARMS);

		return use_random_weight_init_seed ? get_rnd_engine() : weight_init_engine;
	}
}


void Darknet::disable_core_dumps()
{
#ifdef WIN32
	// no-op on Windows in this code path
#else
...
//	TAT(TATPARMS); ... don't bother, we're already about to abort because something has gone wrong, don't make things worse
	...
	*cfg_and_state.output << std::endl << std::flush;
	...
	// tend to hang when things go wrong.  Also do not produce core dumps on fatal exits.
	Darknet::disable_core_dumps();
...
float rand_uniform_weight_init(float min, float max)
{
	...
	return distribution(get_weight_init_engine());
}
...
void rand_uniform_many_weight_init(float * x, const size_t n, float min, float max, const float scale)
{
	TAT(TATPARMS);
	...
	auto & engine = get_weight_init_engine();
...
void Darknet::configure_weight_init_rng(const bool random_init_weights)
{
	TAT(TATPARMS);

	use_random_weight_init_seed = random_init_weights;
	weight_init_engine.seed(k_fixed_weight_init_seed);
}
