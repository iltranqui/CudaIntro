...
void activate_array_eml(float *x, const int n, float * activation_input, float * output)
{
	TAT(TATPARMS);

	constexpr float EML_THRESHOLD = 2.0f;
	const float threshold_exp = expf(EML_THRESHOLD);
	const float threshold_value = threshold_exp - log1pf(threshold_exp);
	constexpr float center_value = 1.0f - 0.6931471805599453f;
	...
	for (i = 0; i < n; ++i) {
		...
		if (x_val > EML_THRESHOLD) {
			output[i] = threshold_value - center_value;
		}
		else {
			const float ex = expf(x_val);
			output[i] = ex - log1pf(ex) - center_value;
		}
	}
}
...
void gradient_array_eml(const int n, const float * activation_input, float * delta)
{
	TAT(TATPARMS);
	...
	for (i = 0; i < n; ++i) {
		...
		if (x > EML_THRESHOLD) {
			...
			grad = std::min((ex * ex) / (1.0f + ex), 1.0f);
		}
		delta[i] *= grad;
	}
}
