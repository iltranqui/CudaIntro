...
#ifndef DARKNET_GPU
	if (l.activation == SWISH || l.activation == MISH || l.activation == EML)
	{
...
void resize_shortcut_layer(Darknet::Layer *l, int w, int h, Darknet::Network * net)
{
	...
	if (l->activation == SWISH || l->activation == MISH || l->activation == EML) l->activation_input = (float*)realloc(l->activation_input, l->batch*l->outputs * sizeof(float));
