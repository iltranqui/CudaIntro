...
namespace
{
	...
	static bool gpu_nan_checks_enabled()
	{
		static const bool env_enabled = std::getenv("DARKNET_GPU_NAN_CHECKS") != nullptr;
		return env_enabled || cfg_and_state.is_trace;
	}

	static bool check_nan_gpu_network(
	...
	{
		if (!gpu_nan_checks_enabled() || d_arr == nullptr || size == 0)
		{
			return false;
		}
		...
		if (is_nan_or_inf(d_arr, size))
		{
			...
			if (delta_owner)
			{
				...
				std::printf("[%s layer] NaN/Inf detected at layer %d, step: %s, delta owner: %s\n",
					layer_label.c_str(), layer.index, step_name, owner_location.c_str());
			}
			else
			{
				std::printf("[%s layer] NaN/Inf detected at layer %d, step: %s\n",
					layer_label.c_str(), layer.index, step_name);
			}
			return true;
		}
...
void forward_network_gpu(Darknet::Network & net, Darknet::NetworkState state)
{
	...
	{
		state.original_input = state.input;
	}
	for (int i = 0; i < net.n; ++i)
	{
		...
		//check_nan_gpu_network("forward: network initial input", state.input, static_cast<size_t>(l.inputs) * l.batch, l);
		l.forward_gpu(l, state);
		check_nan_gpu_network("forward: network output", l.output_gpu, static_cast<size_t>(l.outputs) * l.batch, l);

		if(net.wait_stream)
		{
			CHECK_CUDA(cudaStreamSynchronize(get_network_cuda_stream(net)));
		}
...
void backward_network_gpu(Darknet::Network & net, Darknet::NetworkState state)
{
	...
	if (!state.original_delta)
	{
		state.original_delta = original_delta;
	}
	for(i = net.n-1; i >= 0; --i)
	{
		state.index = i;
		...
		check_nan_gpu_network("backward: network initial delta", l.delta_gpu, static_cast<size_t>(l.outputs) * l.batch, l);
		l.backward_gpu(l, state);
		if (state.delta != nullptr && i > 0)
		{
			const Darknet::Layer & prev = net.layers[i - 1];
			check_nan_gpu_network("backward: network output delta", state.delta, static_cast<size_t>(prev.outputs) * prev.batch, l, &prev);
		}
...
void forward_backward_network_gpu(Darknet::Network & net, float *x, float *y)
{
	...
	for (i = 0; i < net.n; ++i)
	{
		...
		{
			const int cudnn_16bit_mode = net.cudnn_bf16 ? DARKNET_CUDNN_16BIT_BF16 : DARKNET_CUDNN_16BIT_HALF;
			if (l.type == Darknet::ELayerType::CONVOLUTIONAL && l.weights_gpu && l.weights_gpu16)
				...
				else if (l.type == Darknet::ELayerType::CONNECTED && l.weights_gpu && l.weights_gpu16)
				{
					cuda_convert_f32_to_cudnn_16bit(l.weights_gpu, l.inputs * l.outputs, l.weights_gpu16, cudnn_16bit_mode);
				}
				else if (l.type == Darknet::ELayerType::DCNV4 && l.offset_weights_gpu && l.offset_weights_gpu16)
				{
					int K = l.size * l.size;
					if (l.remove_center) K -= 1;
					const int padded_offset_dim = ((l.groups * K * 3 + 7) / 8) * 8;
					const size_t offset_weights_size = static_cast<size_t>(padded_offset_dim) * l.c * l.size * l.size;
					cuda_convert_f32_to_cudnn_16bit(l.offset_weights_gpu, offset_weights_size, l.offset_weights_gpu16, cudnn_16bit_mode);
				}
				else if (l.type == Darknet::ELayerType::CRNN && l.input_layer->weights_gpu && l.input_layer->weights_gpu16)
				{
					...
					cuda_convert_f32_to_cudnn_16bit(l.input_layer->weights_gpu, l.input_layer->nweights, l.input_layer->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.self_layer->weights_gpu, l.self_layer->nweights, l.self_layer->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.output_layer->weights_gpu, l.output_layer->nweights, l.output_layer->weights_gpu16, cudnn_16bit_mode);
			}
			else if (l.type == Darknet::ELayerType::RNN && l.input_layer && l.input_layer->weights_gpu16)
			{
				cuda_convert_f32_to_cudnn_16bit(l.input_layer->weights_gpu, l.input_layer->inputs * l.input_layer->outputs, l.input_layer->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.self_layer->weights_gpu, l.self_layer->inputs * l.self_layer->outputs, l.self_layer->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.output_layer->weights_gpu, l.output_layer->inputs * l.output_layer->outputs, l.output_layer->weights_gpu16, cudnn_16bit_mode);
			}
			else if (l.type == Darknet::ELayerType::LSTM && l.wf && l.wf->weights_gpu16)
			{
				cuda_convert_f32_to_cudnn_16bit(l.wf->weights_gpu, l.wf->inputs * l.wf->outputs, l.wf->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.wi->weights_gpu, l.wi->inputs * l.wi->outputs, l.wi->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.wg->weights_gpu, l.wg->inputs * l.wg->outputs, l.wg->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.wo->weights_gpu, l.wo->inputs * l.wo->outputs, l.wo->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.uf->weights_gpu, l.uf->inputs * l.uf->outputs, l.uf->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.ui->weights_gpu, l.ui->inputs * l.ui->outputs, l.ui->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.ug->weights_gpu, l.ug->inputs * l.ug->outputs, l.ug->weights_gpu16, cudnn_16bit_mode);
				cuda_convert_f32_to_cudnn_16bit(l.uo->weights_gpu, l.uo->inputs * l.uo->outputs, l.uo->weights_gpu16, cudnn_16bit_mode);
			}
		}
	}
#endif
	...
	// All YOLO heads queue only a tiny metrics copy during forward.  Synchronize
	// once here, after backward has consumed the resident deltas, and publish the
	// costs before train_network_datum_gpu() calls get_network_cost().
...
float *get_network_output_layer_gpu(Darknet::Network & net, int i)
{
	...
	Darknet::Layer & l = net.layers[i];
	// REGION/YOLO and anchor-free heads (CenterNet/YOLOX/PP-YOLOE/YOLO-NAS) compute their output on
	// the CPU and already leave l.output populated, so there is nothing to pull back from the GPU.
...
float *network_predict_gpu(Darknet::Network & net, float *input)
{
	...
	// Use per-network CUDA graph storage instead of static variables for thread safety
	cudaGraphExec_t instance = (cudaGraphExec_t)net.cuda_graph_exec;
	// Relay scale calibration must run eagerly.  Capturing this provisional
	// first frame would freeze the pre-relay graph, so capture begins on the
	// next request after the safe edges have been selected.
	const bool fp8_calibration_frame = net.fp8_activation_calibration_pending != 0;
	const bool capture_this_forward = net.use_cuda_graph == 1 && !fp8_calibration_frame;
	...
	if ((*net.cuda_graph_ready) == 0)
	{
		...
		if (capture_this_forward)
		{
			...
			// Store back into network for future use
			net.cuda_graph = graph;
			net.cuda_graph_exec = instance;
