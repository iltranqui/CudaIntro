...
namespace
{
	...
	static void copy_host_array_if_present(float * dst, const float * src, const size_t count)
	{
	...
	{
		return l.nbiases > 0 ? static_cast<size_t>(l.nbiases) : (l.n > 0 ? static_cast<size_t>(l.n) : 0UL);
	}

	static void copy_trainable_array(float * dst_cpu, float * dst_gpu, const float * src_cpu, const float * src_gpu, const size_t count)
	{
		if (count == 0)
		{
			return;
		...
		copy_host_array_if_present(dst_cpu, src_cpu, count);
	...
	static void copy_trainable_array(float * dst_cpu, const float * src_cpu, const size_t count)
	{
		copy_trainable_array(dst_cpu, nullptr, src_cpu, nullptr, count);
	}

	static void copy_convolutional_trainable_params(const Darknet::Layer & src, Darknet::Layer & dst)
	{
		copy_trainable_array(dst.weights, dst.weights_gpu, src.weights, src.weights_gpu, src.nweights > 0 ? static_cast<size_t>(src.nweights) : 0UL);
		copy_trainable_array(dst.biases, dst.biases_gpu, src.biases, src.biases_gpu, layer_bias_count(src));
		const size_t filters = src.n > 0 ? static_cast<size_t>(src.n) : 0UL;
		copy_trainable_array(dst.scales, dst.scales_gpu, src.scales, src.scales_gpu, filters);
		copy_trainable_array(dst.rolling_mean, dst.rolling_mean_gpu, src.rolling_mean, src.rolling_mean_gpu, filters);
		copy_trainable_array(dst.rolling_variance, dst.rolling_variance_gpu, src.rolling_variance, src.rolling_variance_gpu, filters);
	}

#if defined(DARKNET_GPU) && (defined(DARKNET_HAS_FP8) || defined(DARKNET_HAS_FP4))
	// Shared by the FP8 training path and the FP4 preparation path: the conv
	// layer that feeds directly into a detection head (yolo/anchor-free output)
	// is kept out of low-precision quantization.  Detection heads are the most
	// accuracy-sensitive point in the network, so -- like the very first conv
	// layer, which is excluded for unrelated shape reasons -- the last conv
	// layer before each head stays at full precision automatically, with no
	// cfg option required.
	static bool detection_head_type(const Darknet::ELayerType type)
	{
		return type == Darknet::ELayerType::YOLO ||
			type == Darknet::ELayerType::GAUSSIAN_YOLO ||
			type == Darknet::ELayerType::YOLOX ||
			type == Darknet::ELayerType::PPYOLOE ||
			type == Darknet::ELayerType::YOLONAS ||
			type == Darknet::ELayerType::CENTERNET ||
			type == Darknet::ELayerType::DETR_DECODER;
	}

	static bool layer_feeds_detection_head(const Darknet::Network & net, const int layer_index)
	{
		const int next_index = layer_index + 1;
		return next_index >= 0 && next_index < net.n && detection_head_type(net.layers[next_index].type);
	}
#endif

#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP8)
	static bool fp8_training_candidate(const Darknet::Layer & l)
	{
...
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
	struct Fp4NetworkPreparation
	{
		int candidates = 0;
		int enabled = 0;
		int relay_links = 0;
	...
	// Rebuild all shape-dependent FP4 state after loading/fusion and after a
	// runtime resize.  Inference weights remain FP32 masters but are prepacked
	// once into immutable NVFP4 storage here.  The second pass discovers direct
	// physical conv->conv links and retains their activation in the consumer's
	// cuBLASLt [spatial,K] NVFP4 layout.  The conv layer directly feeding a
	// detection head is always excluded (see layer_feeds_detection_head()),
	// same as the first training conv layer -- kept at full precision
	// regardless of shape eligibility.
	static Fp4NetworkPreparation fp4_prepare_network_convolutions(Darknet::Network & net)
	{
		Fp4NetworkPreparation result;
		bool first_train_conv_seen = false;
		for (int j = 0; j < net.n; ++j)
		{
			Darknet::Layer & l = net.layers[j];
			if (l.type != Darknet::ELayerType::CONVOLUTIONAL) continue;

			++result.candidates;
			const bool first_train_conv = net.fp4_training && !first_train_conv_seen;
			if (net.fp4_training) first_train_conv_seen = true;
			const bool detection_head_feeder = layer_feeds_detection_head(net, j);
			if (first_train_conv || detection_head_feeder)
			{
				// Excluded from FP4 by policy, not by shape -- still release any
				// stale plan/relay state left over from a previous prepare pass
				// instead of leaving it dangling.
				fp4_release_convolutional_layer(l);
			}
			else if (fp4_setup_convolutional_layer(l, net.fp4_training != 0))
			{
				++result.enabled;
			}
		}

		// fp4_setup_convolutional_layer() releases a layer's old relay, but the
		// first training layer is deliberately not rebuilt.  Clear every stale
		// edge before discovering inference-only relay links.
		for (int j = 0; j < net.n; ++j)
		{
			if (net.layers[j].type == Darknet::ELayerType::CONVOLUTIONAL)
			{
				fp4_clear_convolutional_relay(net.layers[j]);
			}
		}

		if (net.fp4_inference && !net.fp4_training)
		{
			for (int j = 0; j + 1 < net.n; ++j)
			{
				Darknet::Layer & producer = net.layers[j];
				Darknet::Layer & consumer = net.layers[j + 1];
				if (producer.type == Darknet::ELayerType::CONVOLUTIONAL &&
					consumer.type == Darknet::ELayerType::CONVOLUTIONAL &&
					fp4_setup_convolutional_relay(producer, consumer, j, j + 1))
				{
					++result.relay_links;
				}
			}
		...
		// Tucker attention Q@K^T / attn@V (forward) and dAttn/dV/dQ/dK (backward)
		// GEMMs: opt-in, additive, diagnostic-only (see darknet_layers.hpp's
		// fp4_tucker_* field comments and tucker_attention_kernels.cu).  Plans are
		// created unconditionally at layer setup, so enabling only needs them to exist.
		for (int j = 0; j < net.n; ++j)
		{
			Darknet::Layer & l = net.layers[j];
			if (l.type != Darknet::ELayerType::TUCKER_ATTENTION) continue;
			l.fp4_tucker_attention = (l.fp4_tucker_scores_gemm_plan != nullptr && l.fp4_tucker_context_gemm_plan != nullptr) ? 1 : 0;
		}
	...
	// Rebuild all inference-only FP8 state after weights/fusion and after a
	// dynamic resize.  FP32 weights stay authoritative; their E4M3 copies and
	// the optional activation relays are prepared once before the first image.
	static Fp8NetworkPreparation fp8_prepare_network_inference_convolutions(Darknet::Network & net)
	{
		Fp8NetworkPreparation result;
		net.precision_planned_fp8_layers = 0;
		net.precision_bf16_fallback_layers = 0;
		net.fp8_activation_relay_links = 0;
		net.fp8_activation_relay_enabled_links = 0;
		net.fp8_activation_saturation_fallbacks = 0;
		net.fp8_activation_calibration_samples = 0;
		net.fp8_activation_relay_executions = 0;
		net.fp8_graph_fused_executions = 0;
		...
		for (int j = 0; j < net.n; ++j)
		{
			...
			if (l.type != Darknet::ELayerType::CONVOLUTIONAL) continue;
			++result.candidates;
			if (fp8_setup_convolutional_layer(l))
			{
				++result.enabled;
				++net.precision_planned_fp8_layers;
			}
			else if (net.cudnn_bf16)
			{
				++net.precision_bf16_fallback_layers;
			}
		}

		// Tucker attention Q@K^T / attn@V GEMMs: opt-in, additive (see
		// tucker_attention_kernels.cu's forward_tucker_attention_layer_gpu).  The GEMM
		// plans are created unconditionally at layer setup, so enabling only needs the
		// plans to exist; per-tensor calibration is not wired up yet (Phase 1a step 4),
		// so the quantize kernels fall back to an identity (1.0) scale -- diagnostic-only
		// until calibration lands, same caveat already accepted for conv FP4.
		for (int j = 0; j < net.n; ++j)
		{
			Darknet::Layer & l = net.layers[j];
			if (l.type != Darknet::ELayerType::TUCKER_ATTENTION) continue;
			l.fp8_tucker_attention = (l.fp8_tucker_scores_gemm_plan != nullptr && l.fp8_tucker_context_gemm_plan != nullptr) ? 1 : 0;
		}
		...
		for (int j = 0; j < net.n; ++j)
		{
			if (net.layers[j].type == Darknet::ELayerType::CONVOLUTIONAL)
			{
				fp8_clear_convolutional_relay(net.layers[j]);
			}
		}
		if (result.enabled > 0)
		{
			for (int j = 0; j + 1 < net.n; ++j)
			{
				...
				if (producer.type == Darknet::ELayerType::CONVOLUTIONAL &&
					consumer.type == Darknet::ELayerType::CONVOLUTIONAL &&
					fp8_setup_convolutional_relay(producer, consumer, j, j + 1))
				{
					++result.relay_links;
				}
			}
		}
		net.fp8_activation_relay_links = result.relay_links;
		net.fp8_activation_calibration_pending = result.relay_links > 0 ? 1 : 0;
		return result;
	}
#endif

	static void copy_wmhf_trainable_params(const Darknet::Layer & src, Darknet::Layer & dst)
	{
		if (src.type != Darknet::ELayerType::WMHF or dst.type != Darknet::ELayerType::WMHF or src.input_layer == nullptr)
		{
			darknet_fatal_error(DARKNET_LOC, "mAP WMHF layer mismatch while copying weights");
		}

		if (dst.input_layer == nullptr or dst.weights == nullptr or dst.weight_updates == nullptr or dst.output == nullptr or dst.delta == nullptr)
		{
			Darknet::free_wmhf_layer(dst);

			const int filters = src.out_c;
			if (filters <= 0)
			{
				darknet_fatal_error(DARKNET_LOC, "mAP WMHF layer has invalid filter count while copying weights");
			}

			const float identity_ratio = static_cast<float>(src.groups) / static_cast<float>(filters);
			const float local_ratio = static_cast<float>(src.group_id) / static_cast<float>(filters);
			const int batch = dst.batch > 0 ? dst.batch : 1;
			const int index = dst.index >= 0 ? dst.index : src.index;
			const int adam = src.input_layer[0].adam;
			dst = Darknet::make_wmhf_layer(
				batch,
				src.h,
				src.w,
				src.c,
		...
		if (dst.w != src.w or dst.h != src.h)
		{
			Darknet::resize_wmhf_layer(&dst, src.w, src.h);
		}
		...
		{
			copy_convolutional_trainable_params(src.input_layer[i], dst.input_layer[i]);
			dst.input_layer[i].batch = 1;
			dst.input_layer[i].steps = 1;
			dst.input_layer[i].train = 0;
		}
	}

	static bool check_nan_cpu_network(
		const char *step_name,
		const float *arr,
		int n,
		const Darknet::Layer & layer,
		const Darknet::Layer *delta_owner = nullptr)
	{
		if (arr == nullptr || n <= 0)
		{
			return false;
		...
		for (int i = 0; i < n; ++i)
		{
			...
			{
				const std::string layer_label = Darknet::layer_type_diagnostic_label(layer.type);
				if (delta_owner)
				{
					const std::string owner_location = Darknet::layer_diagnostic_location(delta_owner->index, delta_owner->type);
					std::printf("[%s layer] NaN/Inf detected at layer %d, step: %s, delta owner: %s (idx=%d, value=%g)\n",
						layer_label.c_str(), layer.index, step_name, owner_location.c_str(), i, static_cast<double>(value));
				}
				else
				{
					std::printf("[%s layer] NaN/Inf detected at layer %d, step: %s (idx=%d, value=%g)\n",
						layer_label.c_str(), layer.index, step_name, i, static_cast<double>(value));
				}
				return true;
			}
		}
...
void reset_network_state(Darknet::Network *net, int b)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		{
			// CRNN state buffers are hidden*batch*(steps+1) floats, and the recurrent
			// path also reads the self layer's previous output -- zero both, on CPU
			// and GPU, otherwise state leaks from one video into the next
			const int state_size = l.hidden * l.batch * (l.steps + 1);
			if (l.state)
			{
				fill_cpu(state_size, 0, l.state, 1);
			}
			if (l.self_layer and l.self_layer->output)
			{
				fill_cpu(l.self_layer->outputs * l.self_layer->batch, 0, l.self_layer->output, 1);
			}
#ifdef DARKNET_GPU
			if (l.state_gpu)
			{
				fill_ongpu(state_size, 0, l.state_gpu, 1);
			}
			if (l.self_layer and l.self_layer->output_gpu)
			{
				fill_ongpu(l.self_layer->outputs * l.self_layer->batch, 0, l.self_layer->output_gpu, 1);
			}
#endif
			continue;
		}
...
Darknet::Network make_network(int n)
{
	...
	*net.rewritten_bbox = *net.total_bbox = 0;
	...
	// Initialize per-network CUDA resources (lazy initialization later)
	net.cuda_stream = nullptr;
	net.cuda_stream_initialized = 0;
	net.cudnn_handle = nullptr;
	net.cudnn_handle_initialized = 0;
	net.cublas_handle = nullptr;
	net.cublas_handle_initialized = 0;
	net.cublaslt_handle = nullptr;
	net.cublaslt_handle_initialized = 0;
	net.fp8_warmup_iters = 4;
...
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP8)
void fp8_finalize_network_activation_calibration(Darknet::Network & net)
{
	if (!net.fp8_inference || !net.fp8_activation_calibration_pending)
	{
		return;
	}

	int enabled_links = 0;
	for (int index = 0; index + 1 < net.n; ++index)
	{
		Darknet::Layer & producer = net.layers[index];
		if (producer.type != Darknet::ELayerType::CONVOLUTIONAL ||
			producer.fp8_relay_next_layer != index + 1 ||
			producer.fp8_relay_amax_gpu == nullptr)
		{
			continue;
		}
		Darknet::Layer & consumer = net.layers[index + 1];
		const float amax = Darknet::fp8_pull_amax_gpu(producer.fp8_relay_amax_gpu);
		// E4M3FN's finite range is 448.  Hold a small guard band so the
		// calibration frame itself and later ordinary frames do not begin at the
		// saturation cliff.  Input scales are dequantization factors in the
		// cuDNN graph, hence real_value / input_scale is stored in E4M3.
		const float scale = consumer.fp8_input_scale_host;
		const float representable = std::isfinite(scale) && scale > 0.0f ? 448.0f * scale * 0.98f : 0.0f;
		const bool safe = std::isfinite(amax) && amax <= representable;
		producer.fp8_relay_enabled = safe ? 1 : 0;
		producer.fp8_relay_saturation_fallback = safe ? 0 : 1;
		if (safe)
		{
			++enabled_links;
		}
		...
		{
			// This is a precision boundary, not an error.  The next convolution
			// sees the producer's still-valid FP32 output and therefore falls back
			// to its BF16 cuDNN path when available (otherwise its established
			// FP32 path).  Disable its FP8 plan for the rest of this network shape
			// so it cannot repeatedly quantize with a known-bad sidecar scale.
			consumer.fp8_eligible = 0;
			consumer.fp8_relay_source_layer = -1;
			++net.fp8_activation_saturation_fallbacks;
			if (net.cudnn_bf16)
			{
				++net.precision_bf16_fallback_layers;
			}
	...
	net.fp8_activation_relay_enabled_links = enabled_links;
	net.fp8_activation_calibration_pending = 0;
	++net.fp8_activation_calibration_samples;
}
#endif
...
#endif
		//check_nan_cpu_network("forward: network initial input", state.input, l.inputs * l.batch, l);
		l.forward(l, state);
		check_nan_cpu_network("forward: network output", l.output, l.outputs * l.batch, l);
...
void backward_network(Darknet::Network & net, Darknet::NetworkState state)
{
	...
	for (int i = net.n-1; i >= 0; --i)
	{
		...
		check_nan_cpu_network("backward: network initial delta", l.delta, l.outputs * l.batch, l);
		l.backward(l, state);
		if (state.delta != nullptr && i > 0)
		{
			const Darknet::Layer & prev = net.layers[i - 1];
			check_nan_cpu_network("backward: network output delta", state.delta, prev.outputs * prev.batch, l, &prev);
		}
	}
}
...
void set_batch_network(Darknet::Network * net, int b)
{
	...
	for (i = 0; i < net->n; ++i)
	{
		...
		if (net->layers[i].type == Darknet::ELayerType::WMHF)
		{
			resize_wmhf_layer(&net->layers[i], net->layers[i].w, net->layers[i].h);
		}
...
int resize_network(Darknet::Network * net, int w, int h)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		switch (l.type)
		{
			...
			case Darknet::ELayerType::DCNV4:			resize_dcnv4_layer(&l, w, h);			break;
			case Darknet::ELayerType::WMHF:			resize_wmhf_layer(&l, w, h);			break;
			case Darknet::ELayerType::DECONVOLUTIONAL:	resize_deconvolutional_layer(&l, w, h);		break;
			case Darknet::ELayerType::GRAPH_CONV:		resize_graph_conv_layer(&l, w, h);			break;
			case Darknet::ELayerType::TRANSFORMER:		resize_transformer_layer(&l, w, h);			break;
			case Darknet::ELayerType::VIT:				resize_vit_layer(&l, w, h);					break;
			case Darknet::ELayerType::MAMBAVISION:		resize_mambavision_layer(&l, w, h);			break;
			case Darknet::ELayerType::TUCKER_ATTENTION:	resize_tucker_attention_layer(&l, w, h);	break;
			case Darknet::ELayerType::DETR_DECODER:		resize_detr_decoder_layer(&l, w, h);		break;
			case Darknet::ELayerType::CLIFFORD:		resize_clifford_layer(&l, w, h);			break;
			...
			case Darknet::ELayerType::YOLOX:			resize_yolox_layer(&l, w, h);				break;
			case Darknet::ELayerType::PPYOLOE:			resize_ppyoloe_layer(&l, w, h);				break;
			case Darknet::ELayerType::YOLONAS:			resize_yolonas_layer(&l, w, h);				break;
			case Darknet::ELayerType::CENTERNET:		resize_centernet_layer(&l, w, h);			break;
			...
			case Darknet::ELayerType::CHANNEL_SHUFFLE:	resize_channel_shuffle_layer(&l, h, w);		break;
...
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
	// resize_convolutional_layer() correctly drops shape-dependent FP4 plans and
	// relay buffers.  Recreate them before this function allocates the shared
	// workspace so a resized inference network keeps both eager weight packing
	// and every compatible conv->conv retained-NVFP4 link.
	if (cfg_and_state.gpu_index >= 0 && (net->fp4_inference || net->fp4_training))
	{
		const Fp4NetworkPreparation fp4 = fp4_prepare_network_convolutions(*net);
		if (fp4.enabled == 0)
		{
			Darknet::display_warning_msg("FP4 was requested after resize but no convolutional layer plan was supported; using FP8/cuDNN fallbacks.\n");
			net->fp4_inference = 0;
			net->fp4_training = 0;
		}
		else
		{
			for (int i = 0; i < net->n; ++i)
			{
				...
				if (l.type != Darknet::ELayerType::CONVOLUTIONAL) continue;
				l.workspace_size = get_convolutional_workspace_size(l);
				workspace_size = std::max(workspace_size, l.workspace_size);
			}
			if (cfg_and_state.is_verbose)
			{
				*cfg_and_state.output << "FP4 after resize: enabled " << fp4.enabled << " of "
					<< fp4.candidates << " convolutional layers; retained-NVFP4 relay links="
					<< fp4.relay_links << "." << std::endl;
			}
		}
	}
#endif
...
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP8)
	// resize_convolutional_layer() releases shape-dependent FP8 plans and relay
	// allocations.  Rebuild them before allocating the common workspace.
	if (cfg_and_state.gpu_index >= 0 && net->fp8_inference && !net->fp8_calibrating)
	{
		const Fp8NetworkPreparation fp8 = fp8_prepare_network_inference_convolutions(*net);
		if (fp8.enabled == 0)
		{
			Darknet::display_warning_msg("FP8 was requested after resize but no convolutional layer plan was supported; using BF16/FP32 fallbacks.\n");
			net->fp8_inference = 0;
		}
		else
		{
			...
			if (cfg_and_state.is_verbose)
			{
				*cfg_and_state.output << "FP8 after resize: enabled " << fp8.enabled << " of "
					<< fp8.candidates << " convolutional layers; FP8-NHWC relay links="
					<< fp8.relay_links << " (first-frame scale guard pending)." << std::endl;
			}
		}
	}
#endif
...
int num_detections(Darknet::Network * net, float thresh)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		if (l.type == Darknet::ELayerType::YOLOX)	{ s += Darknet::yolox_num_detections(l, thresh); }
		if (l.type == Darknet::ELayerType::PPYOLOE)	{ s += Darknet::ppyoloe_num_detections(l, thresh); }
		if (l.type == Darknet::ELayerType::YOLONAS)	{ s += Darknet::yolonas_num_detections(l, thresh); }
		if (l.type == Darknet::ELayerType::CENTERNET)	{ s += Darknet::centernet_num_detections(l, thresh); }
		if (l.type == Darknet::ELayerType::DETR_DECODER)	{ s += detr_decoder_num_detections(l, thresh); }
	}
...
int num_detections_v3(Darknet::Network * net, float thresh, Darknet::Output_Object_Cache & cache)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		else if (l.type == Darknet::ELayerType::YOLOX)	{ detections += Darknet::yolox_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::PPYOLOE)	{ detections += Darknet::ppyoloe_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::YOLONAS)	{ detections += Darknet::yolonas_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::CENTERNET)	{ detections += Darknet::centernet_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::DETR_DECODER)	{ detections += detr_decoder_num_detections(l, thresh); }
	}
...
int num_detections_batch(Darknet::Network * net, float thresh, int batch)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		else if (l.type == Darknet::ELayerType::YOLOX)		{ s += Darknet::yolox_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::PPYOLOE)	{ s += Darknet::ppyoloe_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::YOLONAS)	{ s += Darknet::yolonas_num_detections(l, thresh); }
		else if (l.type == Darknet::ELayerType::CENTERNET)	{ s += Darknet::centernet_num_detections_batch(l, thresh, batch); }
		else if (l.type == Darknet::ELayerType::DETR_DECODER)	{ s += detr_decoder_num_detections_batch(l, thresh, batch); }
	}
...
DarknetDetection * make_network_boxes(DarknetNetworkPtr ptr, float thresh, int * num)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		if (l_tmp.type == Darknet::ELayerType::YOLO or
			...
			l_tmp.type == Darknet::ELayerType::YOLOX or
			l_tmp.type == Darknet::ELayerType::PPYOLOE or
			l_tmp.type == Darknet::ELayerType::YOLONAS or
			l_tmp.type == Darknet::ELayerType::CENTERNET or
			l_tmp.type == Darknet::ELayerType::DETR_DECODER or
			i == (net->n - 1))
		{
			l = l_tmp;
			break;
	...
	const Darknet::Layer & l = [&net]()
	{
		for (int i = 0; i < net->n; ++i)
		{
			...
			if (tmp.type == Darknet::ELayerType::YOLO			or
				...
				tmp.type == Darknet::ELayerType::REGION			or
				tmp.type == Darknet::ELayerType::YOLOX			or
				tmp.type == Darknet::ELayerType::PPYOLOE		or
				tmp.type == Darknet::ELayerType::YOLONAS		or
				tmp.type == Darknet::ELayerType::CENTERNET		or
				tmp.type == Darknet::ELayerType::DETR_DECODER	)
			{
				return tmp;
			}
		}
...
Darknet::Detection *make_network_boxes_batch(Darknet::Network * net, float thresh, int *num, int batch)
{
	...
	for (int i = 0; i < net->n; ++i)
	{
		...
		if (l_tmp.type == Darknet::ELayerType::YOLO or
			...
			l_tmp.type == Darknet::ELayerType::YOLONAS or
			l_tmp.type == Darknet::ELayerType::CENTERNET or
			l_tmp.type == Darknet::ELayerType::DETR_DECODER)
		{
			l = l_tmp;
			break;
		}
...
void fill_network_boxes(Darknet::Network * net, int w, int h, float thresh, float hier, int *map, int relative, Darknet::Detection *dets, int letter)
{
	...
	for (int j = 0; j < net->n; ++j)
	{
		...
		switch (l.type)
		{
			...
			case Darknet::ELayerType::YOLOX:
			{
				const int count = Darknet::get_yolox_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
				dets += count;
				break;
			}
			case Darknet::ELayerType::PPYOLOE:
			{
				const int count = Darknet::get_ppyoloe_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
				dets += count;
				break;
			}
			case Darknet::ELayerType::YOLONAS:
			{
				const int count = Darknet::get_yolonas_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
				dets += count;
				break;
			}
			case Darknet::ELayerType::CENTERNET:
			{
				const int count = Darknet::get_centernet_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
				dets += count;
				break;
			}
			case Darknet::ELayerType::DETR_DECODER:
			{
				dets += get_detr_decoder_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
				break;
			}
			default:
			{
...
static inline void fill_network_boxes_v3(Darknet::Network * net, int w, int h, float thresh, float hier, int *map, int relative, Darknet::Detection *dets, int letter, Darknet::Output_Object_Cache & cache)
{
	 ...
	 */
	const int count = get_yolo_detections_v3(net, w, h, net->w, net->h, thresh, map, relative, dets, letter, cache);
	dets += count;

	/* The V3 "jazz" cache above only tracks [yolo] layers.  The modern anchor-free heads
	 * (YOLOX / PP-YOLOE / YOLO-NAS) decode their own grid via dedicated extractors, so fill them
	 * here.  num_detections_v3() already counts these layers, so the totals stay consistent.
	 */
	for (int j = 0; j < net->n; ++j)
	{
		const Darknet::Layer & l = net->layers[j];
		int modern_count = 0;
		if (l.type == Darknet::ELayerType::CENTERNET)
		{
			modern_count = Darknet::get_centernet_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
		}
		else if (l.type == Darknet::ELayerType::YOLOX)
		{
			modern_count = Darknet::get_yolox_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
		}
		else if (l.type == Darknet::ELayerType::PPYOLOE)
		{
			modern_count = Darknet::get_ppyoloe_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
		}
		else if (l.type == Darknet::ELayerType::YOLONAS)
		{
			modern_count = Darknet::get_yolonas_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
		}
		else if (l.type == Darknet::ELayerType::DETR_DECODER)
		{
			modern_count = get_detr_decoder_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
		}
		else
		{
			continue;
...
void fill_network_boxes_batch(Darknet::Network * net, int w, int h, float thresh, float hier, int *map, int relative, Darknet::Detection *dets, int letter, int batch)
{
	...
	for (int j = 0; j < net->n; ++j)
	{
		...
		else if (l.type == Darknet::ELayerType::CENTERNET)
		{
			const int count = Darknet::get_centernet_detections_batch(l, w, h, net->w, net->h, thresh, map, relative, dets, letter, batch);
			dets += count;
		}
		else if (l.type == Darknet::ELayerType::DETR_DECODER)
		{
			dets += get_detr_decoder_detections_batch(l, w, h, net->w, net->h, thresh, map, relative, dets, letter, batch);
		}
	}
}
...
void free_network(Darknet::Network & net)
{
	...
	net.fp8_skip_layers = nullptr;
	net.fp8_skip_layer_count = 0;
	...
	if (net.cuda_graph_exec)
	{
		cudaGraphExecDestroy((cudaGraphExec_t)net.cuda_graph_exec);
		net.cuda_graph_exec = nullptr;
	}
	if (net.cuda_graph)
	{
		cudaGraphDestroy((cudaGraph_t)net.cuda_graph);
		net.cuda_graph = nullptr;
	}

	// Cleanup per-network CUDA resources
	if (net.cuda_stream_initialized)
	{
		cudaStreamDestroy((cudaStream_t)net.cuda_stream);
		net.cuda_stream_initialized = 0;
		net.cuda_stream = nullptr;
	}
#ifdef CUDNN
	if (net.cudnn_handle_initialized)
	{
		cudnnDestroy((cudnnHandle_t)net.cudnn_handle);
		net.cudnn_handle_initialized = 0;
		net.cudnn_handle = nullptr;
	}
#endif
	if (net.cublas_handle_initialized)
	{
		cublasDestroy((cublasHandle_t)net.cublas_handle);
		net.cublas_handle_initialized = 0;
		net.cublas_handle = nullptr;
	}
#ifdef DARKNET_GPU_CUDA
	if (net.cublaslt_handle_initialized)
	{
		cublasLtDestroy((cublasLtHandle_t)net.cublaslt_handle);
		net.cublaslt_handle_initialized = 0;
		net.cublaslt_handle = nullptr;
	}
#endif
...
void calculate_binary_weights(DarknetNetworkPtr ptr)
{
	...
	Darknet::Network & net = *reinterpret_cast<Darknet::Network *>(ptr);
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP8)
	int fp8_candidates = 0;
	int fp8_train_candidates = 0;
	int fp8_train_enabled = 0;
	bool fp8_first_train_conv_seen = false;
#endif
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
	int fp4_calibration_candidates = 0;
#endif
	...
	for (int j = 0; j < net.n; ++j)
	{
		...
		if (l->type == Darknet::ELayerType::CONVOLUTIONAL)
		{
			...
			if (cfg_and_state.gpu_index >= 0)
			{
				...
				if (net.fp8_calibrating)
				{
					if (fp8_prepare_convolutional_calibration_layer(*l))
					{
						fp8_candidates += 1;
					}
				}
				if (net.fp8_training && fp8_training_candidate(*l))
				{
					fp8_train_candidates += 1;
					const bool first_train_conv = !fp8_first_train_conv_seen;
					fp8_first_train_conv_seen = true;
					const bool explicitly_skipped = Darknet::fp8_layer_is_skipped(j, net.n, net.fp8_skip_layers, net.fp8_skip_layer_count);
					const bool detection_head_feeder = layer_feeds_detection_head(net, j);
					if (!first_train_conv && !explicitly_skipped && !detection_head_feeder && fp8_setup_convolutional_training_layer(*l))
					{
						fp8_train_enabled += 1;
					}
				}
...
#endif
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
			if (cfg_and_state.gpu_index >= 0 && net.fp4_calibrating)
			{
				if (fp4_prepare_convolutional_calibration_layer(*l))
				{
					fp4_calibration_candidates += 1;
				}
			}
#endif
		}
...
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
	if (cfg_and_state.gpu_index >= 0 && net.fp4_calibrating && fp4_calibration_candidates > 0 && cfg_and_state.is_verbose)
	{
		*cfg_and_state.output << "FP4 calibration: tracking activation amax for " << fp4_calibration_candidates << " convolutional layers." << std::endl;
	}
#endif
#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP4)
	if (cfg_and_state.gpu_index >= 0 && (net.fp4_inference || net.fp4_training))
	{
		...
		if (fp4.enabled == 0)
		{
			Darknet::display_warning_msg("FP4 was requested but no convolutional layer plan was supported; using FP8/cuDNN fallbacks.\n");
			net.fp4_inference = 0;
			net.fp4_training = 0;
		}
		else if (cfg_and_state.is_verbose)
		{
			*cfg_and_state.output << "FP4: enabled " << fp4.enabled << " of " << fp4.candidates
				<< " convolutional layers; retained-NVFP4 relay links=" << fp4.relay_links << "." << std::endl;
		}
		net.precision_planned_fp4_layers = fp4.enabled;
		if (fp4.enabled > 0) recalculate_workspace_size(&net);
	}
#endif

#if defined(DARKNET_GPU) && defined(DARKNET_HAS_FP8)
	Fp8NetworkPreparation fp8;
	if (cfg_and_state.gpu_index >= 0 && net.fp8_inference && !net.fp8_calibrating)
	{
		fp8 = fp8_prepare_network_inference_convolutions(net);
	}
	if (cfg_and_state.gpu_index >= 0)
	{
		if (net.fp8_calibrating && fp8_candidates > 0)
		{
			if (cfg_and_state.is_verbose)
			{
				*cfg_and_state.output << "FP8 calibration: tracking activation amax for " << fp8_candidates << " convolutional layers." << std::endl;
			}
		}
		else if (net.fp8_inference)
		{
			if (fp8.enabled == 0)
			{
				Darknet::display_warning_msg("FP8 was requested but no convolutional layers were enabled; using the existing inference path.\n");
				net.fp8_inference = 0;
			}
			else
			{
				if (cfg_and_state.is_verbose)
				{
					*cfg_and_state.output << "FP8 inference: enabled " << fp8.enabled << " of " << fp8.candidates
						<< " convolutional layers; FP8-NHWC relay links=" << fp8.relay_links
						<< " (first-frame scale guard pending)." << std::endl;
				}
			}
		...
		if (net.fp8_training)
		{
			if (fp8_train_enabled == 0)
			{
				Darknet::display_warning_msg("FP8 training was requested but no convolutional layers were eligible after skip/head exclusions; using the existing training path.\n");
				net.fp8_training = 0;
			}
			else if (cfg_and_state.is_verbose)
			{
				*cfg_and_state.output << "FP8 training: marked " << fp8_train_enabled << " of " << fp8_train_candidates << " convolutional layers eligible." << std::endl;
			}
			if (fp8_train_enabled > 0)
			{
				recalculate_workspace_size(&net);
...
void copy_weights_net(const Darknet::Network & net_train, Darknet::Network * net_map)
{
	...
	for (int k = 0; k < net_train.n; ++k)
	{
		...
		{
			copy_wmhf_trainable_params(*l, net_map->layers[k]);
...
void reject_similar_weights(Darknet::Network & net, float sim_threshold)
{
	...
	// Skip the convolutional layers that immediately feed a detection head so we do not perturb their
	// weights.  This covers the classic [yolo] head as well as the modern anchor-free heads.
	const auto is_detection_head = [](Darknet::ELayerType t)
	{
		return t == Darknet::ELayerType::YOLO
			or t == Darknet::ELayerType::YOLOX
			or t == Darknet::ELayerType::PPYOLOE
			or t == Darknet::ELayerType::YOLONAS
			or t == Darknet::ELayerType::CENTERNET;
	};
	...
	for (int i = 0; i < net.n; ++i)
	{
		...
		if (net.n > i + 1) if (is_detection_head(net.layers[i + 1].type)) continue;
		if (net.n > i + 2) if (is_detection_head(net.layers[i + 2].type)) continue;
		if (net.n > i + 3) if (is_detection_head(net.layers[i + 3].type)) continue;
