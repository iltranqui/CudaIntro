...
namespace
{
	...
	YoloCudaMode yolo_cuda_mode()
	{
		const char * value = std::getenv("DARKNET_YOLO_TRAINING_GPU");
		if (!value || value[0] == '\0' || std::strcmp(value, "auto") == 0)
		{
			return YoloCudaMode::automatic;
		}
		if (std::strcmp(value, "cpu") == 0) return YoloCudaMode::cpu;
		if (std::strcmp(value, "require") == 0) return YoloCudaMode::require;
		if (std::strcmp(value, "verify") == 0) return YoloCudaMode::verify;
		darknet_fatal_error(DARKNET_LOC,
			"invalid DARKNET_YOLO_TRAINING_GPU value '%s'; expected auto, cpu, require, or verify", value);
		return YoloCudaMode::automatic;
	}
#endif
	...
	static inline ious delta_yolo_box(const Darknet::Box & truth, const float * x, const float * biases, const int n, const int index, const int i, const int j, const int lw, const int lh, const int w, const int h, float * delta, const float scale, const int stride, const float iou_normalizer, const IOU_LOSS iou_loss, const int accumulate, const float max_delta, int * rewritten_bbox, const int new_coords)
	{
		...
		if (iou_loss == MSE)    // old loss
		{
			...
			const float box_scale = scale;

			// accumulate delta
			delta[index + 0 * stride] += box_scale * (tx - x[index + 0 * stride]) * iou_normalizer;
			delta[index + 1 * stride] += box_scale * (ty - x[index + 1 * stride]) * iou_normalizer;
			delta[index + 2 * stride] += box_scale * (tw - x[index + 2 * stride]) * iou_normalizer;
			delta[index + 3 * stride] += box_scale * (th - x[index + 3 * stride]) * iou_normalizer;
		}
		else
		{
...
#ifdef DARKNET_GPU_CUDA
	Darknet::resize_yolo_training_cuda(*l);
	l->yolo_training_gpu_disabled = 0;
#endif
...
void process_batch(void* ptr)
{
		...
		for (int i = 0; i < l.w; ++i)
		{
			for (int n = 0; n < l.n; ++n)
			{
				...
				for (int t = 0; t < l.max_boxes; ++t)
				{
					...
					float iou = box_iou(pred_metric, truth);
	...
	for (int t = 0; t < l.max_boxes; ++t)
	{
		...
		float best_iou = 0;
		int best_n = 0;
		int i = std::max(0, std::min(l.w - 1, static_cast<int>(truth.x * l.w)));
		int j = std::max(0, std::min(l.h - 1, static_cast<int>(truth.y * l.h)));
...
namespace
{
	void forward_yolo_layer_gpu_cpu_fallback(Darknet::Layer & l, Darknet::NetworkState state)
	{
		float * activated_output = static_cast<float *>(xcalloc(l.batch * l.outputs, sizeof(float)));
		cuda_pull_array(l.output_gpu, activated_output, l.batch * l.outputs);
		...
		cpu_state.input = activated_output;
...
#ifdef DARKNET_GPU_CUDA
	void verify_yolo_layer_cuda(Darknet::Layer & l, Darknet::NetworkState state)
	{
		constexpr float absolute_tolerance = 2.0e-5f;
		constexpr float relative_tolerance = 2.0e-4f;
		std::vector<float> cuda_delta(static_cast<size_t>(l.batch) * l.outputs);
		cuda_pull_array(l.delta_gpu, cuda_delta.data(), cuda_delta.size());
		const int total_bbox_before = state.net.total_bbox ? *state.net.total_bbox : 0;
		const int rewritten_bbox_before = state.net.rewritten_bbox ? *state.net.rewritten_bbox : 0;
		Darknet::finalize_yolo_training_cuda(state.net);
		const float cuda_cost = *l.cost;
		const int cuda_total_bbox = state.net.total_bbox
			? *state.net.total_bbox - total_bbox_before : 0;
		const int cuda_rewritten_bbox = state.net.rewritten_bbox
			? *state.net.rewritten_bbox - rewritten_bbox_before : 0;
		if (state.net.total_bbox) *state.net.total_bbox = total_bbox_before;
		if (state.net.rewritten_bbox) *state.net.rewritten_bbox = rewritten_bbox_before;
		forward_yolo_layer_gpu_cpu_fallback(l, state);
		const float cpu_cost = *l.cost;
		const int cpu_total_bbox = state.net.total_bbox
			? *state.net.total_bbox - total_bbox_before : 0;
		const int cpu_rewritten_bbox = state.net.rewritten_bbox
			? *state.net.rewritten_bbox - rewritten_bbox_before : 0;

		float maximum_absolute_error = 0.0f;
		size_t maximum_error_index = 0;
		bool delta_mismatch = false;
		for (size_t index = 0; index < cuda_delta.size(); ++index)
		{
			const float error = std::fabs(cuda_delta[index] - l.delta[index]);
			const float tolerance = absolute_tolerance
				+ relative_tolerance * std::fabs(l.delta[index]);
			delta_mismatch = delta_mismatch || !std::isfinite(cuda_delta[index])
				|| !std::isfinite(l.delta[index]) || error > tolerance;
		...
		*cfg_and_state.output
			<< "CUDA YOLO verification: layer=" << state.index
			<< ", max_abs_error=" << maximum_absolute_error
			<< ", index=" << maximum_error_index
			<< ", gpu=" << cuda_delta[maximum_error_index]
			<< ", cpu=" << l.delta[maximum_error_index]
				<< ", CPU delta is authoritative"
				<< std::endl;

		const float cost_tolerance = 1.0e-4f + relative_tolerance * std::fabs(cpu_cost);
		if (delta_mismatch || !std::isfinite(cuda_cost) || !std::isfinite(cpu_cost)
			|| std::fabs(cuda_cost - cpu_cost) > cost_tolerance
			|| cuda_total_bbox != cpu_total_bbox
			|| cuda_rewritten_bbox != cpu_rewritten_bbox)
		{
			darknet_fatal_error(DARKNET_LOC,
				"CUDA YOLO verification failed: layer=%d max_delta_error=%g "
				"gpu_cost=%g cpu_cost=%g gpu_bbox=%d cpu_bbox=%d "
				"gpu_rewritten=%d cpu_rewritten=%d",
				state.index, maximum_absolute_error, cuda_cost, cpu_cost,
				cuda_total_bbox, cpu_total_bbox,
				cuda_rewritten_bbox, cpu_rewritten_bbox);
		}
	}
...
void forward_yolo_layer_gpu(Darknet::Layer & l, Darknet::NetworkState state)
{
	...
	Darknet::yolo_activate_output_gpu(
		state.input, l.output_gpu, l.batch, l.n, l.w, l.h, l.classes, l.scale_x_y, l.new_coords != 0);
#else
	...
	for (int b = 0; b < l.batch; ++b)
	{
		for (int n = 0; n < l.n; ++n)
		{
			const int bbox_index = yolo_entry_index(l, b, n*l.w*l.h, 0);
			if (!l.new_coords)
			{
				activate_array_ongpu(l.output_gpu + bbox_index, 2 * l.w*l.h, LOGISTIC);
				const int obj_index = yolo_entry_index(l, b, n*l.w*l.h, 4);
				activate_array_ongpu(l.output_gpu + obj_index, (1 + l.classes)*l.w*l.h, LOGISTIC);
			}
			if (l.scale_x_y != 1)
			{
				scal_add_ongpu(2 * l.w*l.h, l.scale_x_y,
					-0.5*(l.scale_x_y - 1), l.output_gpu + bbox_index, 1);
			}
		}
	}
#endif
...
#ifdef DARKNET_GPU_CUDA
	const YoloCudaMode mode = yolo_cuda_mode();
	if (mode != YoloCudaMode::cpu && !l.yolo_training_gpu_disabled)
	{
		const char * reason = nullptr;
		const Darknet::YoloCudaLaunchStatus status = Darknet::forward_yolo_training_cuda(l, state, &reason);
		if (status == Darknet::YoloCudaLaunchStatus::launched)
		{
			if (mode == YoloCudaMode::verify)
			{
				verify_yolo_layer_cuda(l, state);
			}
			return;
		}
		if (mode == YoloCudaMode::require)
		{
			darknet_fatal_error(DARKNET_LOC, "CUDA YOLO training is required but unavailable: %s",
				reason ? reason : "unknown setup error");
		}
		if (status == Darknet::YoloCudaLaunchStatus::recoverable_failure)
		{
			l.yolo_training_gpu_disabled = 1;
			*cfg_and_state.output << "CUDA YOLO setup failed; using CPU fallback: "
				<< (reason ? reason : "unknown setup error") << std::endl;
		}
	}
	else if (mode == YoloCudaMode::require)
	{
		darknet_fatal_error(DARKNET_LOC, "CUDA YOLO training was disabled after a setup failure");
	}
#endif
	forward_yolo_layer_gpu_cpu_fallback(l, state);
}
