...
namespace Darknet
{
	...
	enum class ELayerType
	{
		...
		DECONVOLUTIONAL								,	///< transposed convolution layer
		...
		CHANNEL_SLICE								,	///< slice a contiguous channel range from another layer
		CHANNEL_SHUFFLE								,	///< shuffle channels by groups
		EMPTY			/* aka "SILENCE" */			,	///< *UNUSED*
		BLANK										,	///< *UNUSED*
		CONTRASTIVE								,	///< only used in yolov4-tiny_contrastive.cfg
		GRAPH_CONV								,	///< local spatial graph convolution layer
		DEFORM_CONV								,	///< deformable convolution layer (DCNv2)
		TRANSFORMER								,	///< Swin-style windowed transformer layer
		VIT										,	///< Vision Transformer (Global) layer
		MAMBAVISION								,	///< MambaVision hybrid selective-scan mixer layer
		TUCKER_ATTENTION						,	///< Tucker-factorized windowed attention layer
		CLIFFORD								,	///< Clifford / geometric algebra interaction layer
		DCNV4									,	///< Deformable Convolution v4 layer
		EML_CONV								,	///< binary EML convolution block
		RECURSIVE_BLOCK							,	///< shared-weight depth-recurrent body block
		WMHF									,	///< Wavelet-Mamba high-frequency fusion layer
		YOLOX									,	///< anchor-free YOLOX detection head
		PPYOLOE									,	///< anchor-free PP-YOLOE detection head (DFL + TAL)
		YOLONAS									,	///< anchor-free YOLO-NAS-style detection head
		CENTERNET								,	///< CenterNet-style heatmap detection head
		DETR_DECODER							,	///< DETR-style query-based set-prediction detection head
		LAYER_LAST_IDX	= ELayerType::DETR_DECODER,	///< point to the last-used idx
	};
	...
	std::string layer_type_diagnostic_label(const ELayerType type);
	std::string layer_diagnostic_location(const int layer_index, const ELayerType type);
	/// @}
