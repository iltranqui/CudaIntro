...
std::string Darknet::format_layer_summary(const size_t idx, const Darknet::CfgSection & section, const Darknet::Layer & l)
{
	...
	const std::string size = [&]()
	{
		...
		else if (l.type == Darknet::ELayerType::CENTERNET or
			l.type == Darknet::ELayerType::YOLOX or
			l.type == Darknet::ELayerType::PPYOLOE or
			l.type == Darknet::ELayerType::YOLONAS)
		{
			ss << "anchor-free, " << l.classes << " classes";
		}
		else
		{
			if (l.size > 0)
	...
	// Static shape check only -- mirrors two separate gates in src-fp4/fp4_gemm.cpp:
	//   - fp4_gemm_plan_create()'s reduction%16 gate (the cuDNN Frontend graph
	//     path -- only requires K=c*size*size divisible by 16; M/N unconstrained).
	//   - setup_cublaslt_nvfp4()'s stricter gate (the fast raw cuBLASLt path --
	//     additionally requires K%32==0, M(filters)%8==0, N(spatial)%8==0; batch
	//     is always 1 here since fp4_setup_convolutional_layer() always calls
	//     fp4_gemm_plan_create with batch=1).
	// A layer can satisfy the first but not the second (e.g. every YOLO
	// detection head, M=filters=255 which fails %8) -- it still gets FP4
	// inference, just via the slower cuDNN Frontend fallback instead of the
	// fast cuBLASLt path, so that case is reported distinctly rather than
	// collapsed into a single misleading "ok". Keep this in sync with those
	// gates if they change. This does NOT confirm the GPU/cuDNN combo can
	// actually build a plan -- it only tells you whether the layer's shape
	// rules FP4 (or the fast path) out before that is even attempted.
	const std::string fp4_note = [&]()
	{
		...
		const bool base_eligible =
			...
			!l.binary and
			!l.xnor and
			l.share_layer == nullptr and
			l.c > 0 and l.size > 0 and l.out_h > 0 and l.out_w > 0 and l.n > 0;
		if (!base_eligible)
		{
			return std::string(" fp4-shape-no");
		}

		const long long k = static_cast<long long>(l.c) * l.size * l.size;
		const long long m = l.n;
		const long long spatial = static_cast<long long>(l.out_h) * l.out_w;

		const bool cudnn_frontend_ok = (k % 16 == 0);
		if (!cudnn_frontend_ok)
		{
			return std::string(" fp4-shape-no");
		}

		const bool cublaslt_ok = (k % 32 == 0) and (m % 8 == 0) and (spatial % 8 == 0);

		return cublaslt_ok ? std::string(" fp4-shape-ok") : std::string(" fp4-shape-slow");
	}();

	// Static shape check only -- mirrors fp8_convolutional_candidate()'s structural
	// gate in src-lib/convolutional_layer.cpp:1137-1150 (type/groups/binary/xnor/
	// share_layer), which is the only part of FP8 eligibility knowable from the cfg
	// alone. Unlike FP4, no real K/M/N alignment requirement was found for FP8's
	// cuBLASLt TN matmul (see src-fp8/fp8_gemm.hpp's buffer contract comment), so
	// there is no shape-based "no" beyond the structural gate below. Two further
	// gates exist but are NOT knowable here because this function runs at cfg-parse
	// time, before weights/GPU state exist: whether the GPU has compute capability
	// >= 8.9 (Darknet::fp8_gemm_supported()), and whether a calibration sidecar
	// (.fp8scales) has been loaded (l.fp8_scales_loaded). A "fp8-shape-ok" layer can
	// still end up not running FP8 for either of those reasons.
	const std::string fp8_note = [&]()
	{
		if (l.type != Darknet::ELayerType::CONVOLUTIONAL)
		{
			return std::string();
		}
		const bool structurally_eligible =
			l.groups == 1 and
			!l.binary and
			!l.xnor and
			l.share_layer == nullptr and
		...
		return structurally_eligible ? std::string(" fp8-shape-ok") : std::string(" fp8-no");
	}();
	...
	const std::string output =
		...
		format_in_colour(fp4_note				, EColour::kYellow			, 16) +
		format_in_colour(fp8_note				, EColour::kCyan			, 14);
