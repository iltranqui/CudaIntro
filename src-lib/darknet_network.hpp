...
namespace Darknet
{
	...
	struct Network
	{
		public:
			...
			int fp8_inference;
			int fp8_calibrating;
			int fp8_force_enable;
			int fp8_training;
			int fp8_warmup_iters;
			int *fp8_skip_layers;
			int fp8_skip_layer_count;
			// FP8 activation relay policy is calibrated on the first eager inference
			// so stale/mismatched sidecar scales never silently saturate an edge.
			int fp8_activation_calibration_pending;
			int fp8_activation_calibration_samples;
			int fp8_activation_relay_links;
			int fp8_activation_relay_enabled_links;
			int fp8_activation_saturation_fallbacks;
			int precision_planned_fp8_layers;
			int precision_planned_fp4_layers;
			int precision_bf16_fallback_layers;
			unsigned long long fp8_activation_relay_executions;
			unsigned long long fp8_graph_fused_executions;
			int tf32;
			...
			// Per-network CUDA resources for thread-safe inference
			void *cuda_stream;              ///< cudaStream_t stored as void* for header compatibility
			int cuda_stream_initialized;
			void *cudnn_handle;             ///< cudnnHandle_t stored as void* for header compatibility
			int cudnn_handle_initialized;
			void *cublas_handle;            ///< cublasHandle_t stored as void* for header compatibility
			int cublas_handle_initialized;
			void *cublaslt_handle;          ///< cublasLtHandle_t stored as void* for header compatibility
			int cublaslt_handle_initialized;

			int optimized_memory;
			int dynamic_minibatch;
			int validation_threads;			///< Number of threads to use for validation prediction (default 1, max 8)
...
/// Resolve first-frame amax measurements and enable only non-saturating FP8
/// activation relays.  Call after the network CUDA stream has completed.
