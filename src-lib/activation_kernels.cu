...
static bool is_aligned_16(const void *ptr)
{
...
__device__ float eml_activate_kernel(float x)
{
	constexpr float EML_THRESHOLD = 2.0f;
	const float threshold_exp = expf(EML_THRESHOLD);
	if (x > EML_THRESHOLD) {
		return threshold_exp - log1pf(threshold_exp);
	}
	const float ex = expf(x);
	return ex - log1pf(ex);
}

__global__ void activate_array_eml_kernel(float *x, int n, float *activation_input_gpu, float *output_gpu)
{
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		const float x_val = x[i];
		if (activation_input_gpu) activation_input_gpu[i] = x_val;
		output_gpu[i] = eml_activate_kernel(x_val);
	}
}
...
__global__ void activate_array_leaky_x4_kernel(float4 *x, int n4)
{
	int index = blockIdx.x*blockDim.x + threadIdx.x;
	if (index < n4) {
		float4 v = x[index];
		v.x = leaky_activate_kernel(v.x);
		v.y = leaky_activate_kernel(v.y);
		v.z = leaky_activate_kernel(v.z);
		v.w = leaky_activate_kernel(v.w);
		x[index] = v;
	}
}
...
__device__ float eml_gradient_kernel(float x)
{
	constexpr float EML_THRESHOLD = 2.0f;
	if (x > EML_THRESHOLD) return 0.0f;
	const float ex = expf(x);
	return fminf((ex * ex) / (1.0f + ex), 1.0f);
}

__global__ void gradient_array_eml_kernel(int n, float *activation_input_gpu, float *delta)
{
	int i = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
	if (i < n) {
		delta[i] *= eml_gradient_kernel(activation_input_gpu[i]);
	}
}
...
__global__ void gradient_array_leaky_x4_kernel(const float4 *x, int n4, float4 *delta)
{
	int index = blockIdx.x*blockDim.x + threadIdx.x;
	if (index < n4) {
		const float4 xv = x[index];
		float4 dv = delta[index];
		dv.x *= leaky_gradient_kernel(xv.x);
		dv.y *= leaky_gradient_kernel(xv.y);
		dv.z *= leaky_gradient_kernel(xv.z);
		dv.w *= leaky_gradient_kernel(xv.w);
		delta[index] = dv;
	}
}
...
void activate_array_ongpu(float *x, int n, ACTIVATION a)
{
	...
	if (a == LINEAR) return;
	else if (a == LEAKY || a == REVLEAKY)
	{
		if (n > 0 && (n % 4) == 0 && is_aligned_16(x))
			activate_array_leaky_x4_kernel <<<get_number_of_blocks(n / 4, BLOCK), BLOCK, 0, get_cuda_stream() >>>(reinterpret_cast<float4 *>(x), n / 4);
		else
			activate_array_leaky_kernel <<<num_blocks, BLOCK, 0, get_cuda_stream() >>>(x, n);
	}
...
void activate_array_eml_ongpu(float *x, int n, float *activation_input_gpu, float *output_gpu)
{
	TAT(TATPARMS);

	activate_array_eml_kernel <<<cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >>>(x, n, activation_input_gpu, output_gpu);
	CHECK_CUDA(cudaPeekAtLastError());
}
...
void gradient_array_ongpu(float *x, int n, ACTIVATION a, float *delta)
{
	...
	{
		if (n > 0 && (n % 4) == 0 && is_aligned_16(x) && is_aligned_16(delta))
			gradient_array_leaky_x4_kernel <<<get_number_of_blocks(n / 4, BLOCK), BLOCK, 0, get_cuda_stream() >>>(reinterpret_cast<const float4 *>(x), n / 4, reinterpret_cast<float4 *>(delta));
		else
			gradient_array_leaky_kernel <<<num_blocks, BLOCK, 0, get_cuda_stream() >>>(x, n, delta);
	}
...
void gradient_array_eml_ongpu(int n, float *activation_input_gpu, float *delta)
{
	TAT(TATPARMS);

	gradient_array_eml_kernel <<<cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >>> (n, activation_input_gpu, delta);
	CHECK_CUDA(cudaPeekAtLastError());
}
