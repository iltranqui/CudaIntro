...
namespace Darknet
{
	...
	class CfgSection final
	{
		public:
			...
			bool		consumed = false;	///< True if this section is a body layer inside a [recursive_block] and has already been parsed.
	};
	...
	class CfgFile final
	{
		...
		private:
			...
			Darknet::Layer	parse_eml_convolutional_section(const size_t section_idx);
			Darknet::Layer	parse_deconvolutional_section(const size_t section_idx);
			...
			Darknet::Layer	parse_channel_shuffle_section(const size_t section_idx);
			...
			Darknet::Layer	parse_vit_section			(const size_t section_idx);
			Darknet::Layer	parse_mambavision_section	(const size_t section_idx);
			Darknet::Layer	parse_tucker_attention_section(const size_t section_idx);
			...
			Darknet::Layer	parse_graph_conv_section	(const size_t section_idx);
			Darknet::Layer	parse_deform_conv_section	(const size_t section_idx);
			Darknet::Layer	parse_dcnv4_section			(const size_t section_idx);
			Darknet::Layer	parse_wmhf_section			(const size_t section_idx);
			Darknet::Layer	parse_yolox_section			(const size_t section_idx);
			Darknet::Layer	parse_ppyoloe_section		(const size_t section_idx);
			Darknet::Layer	parse_yolonas_section		(const size_t section_idx);
			Darknet::Layer	parse_detr_decoder_section	(const size_t section_idx);
			Darknet::Layer	parse_transformer_section	(const size_t section_idx);
			Darknet::Layer	parse_clifford_section		(const size_t section_idx);
			Darknet::Layer	parse_recursive_block_section(const size_t section_idx);
			Darknet::Layer	parse_one_body_section		(const size_t section_idx);
			Darknet::Layer	parse_centernet_section		(const size_t section_idx);
			/// @}
	};
}
