...
#ifdef DARKNET_GPU
/// Per-thread CUDA resource overrides.  When non-null, get_cuda_stream() and blas_handle() (and
/// cudnn_handle()) return these values instead of the global device-indexed singletons.  This lets
/// each prediction thread route its kernels to its own network stream without touching any other
/// call site.  Set once at thread start via activate_network_streams(); never needs clearing because
/// thread-local storage is naturally scoped to the thread's lifetime.
thread_local cudaStream_t   tl_cuda_stream_override   = nullptr;
thread_local cublasHandle_t tl_cublas_handle_override = nullptr;
#ifdef DARKNET_GPU_CUDA
thread_local cublasLtHandle_t tl_cublaslt_handle_override = nullptr;
#endif
#ifdef CUDNN
thread_local cudnnHandle_t  tl_cudnn_handle_override  = nullptr;
#endif
#endif // DARKNET_GPU
...
cudaStream_t get_cuda_stream()
{
	...
	// If a prediction thread has activated its own per-network stream, use that instead of the
	// global device-indexed singleton.  This is what allows N prediction threads on the same GPU
	// to dispatch kernels to N independent streams without any coordination overhead.
	if (tl_cuda_stream_override) return tl_cuda_stream_override;
...
cudnnHandle_t cudnn_handle()
{
	...
	if (tl_cudnn_handle_override) return tl_cudnn_handle_override;
...
void cublas_check_error(cublasStatus_t status)
{
	...
	if (status != CUBLAS_STATUS_SUCCESS)
	{
		...
		*cfg_and_state.output << "cuBLAS Error: status=" << status << " (" << status_str << ")" << std::endl;
	}
}
...
#ifdef DARKNET_GPU_CUDA
static int cublasLtInit[16] = { 0 };
static cublasLtHandle_t cublasLtHandle[16];
#endif
...
void init_cublas()
{
	...
	if (!blasInit[i])
	{
		cublasStatus_t status = cublasCreate(&blasHandle[i]);
		if (status != CUBLAS_STATUS_SUCCESS)
		{
			*cfg_and_state.output << "Warning: cuBLAS initialization failed (status=" << status << "), will retry later" << std::endl;
			return;
		}
		status = cublasSetStream(blasHandle[i], get_cuda_stream());
		if (status != CUBLAS_STATUS_SUCCESS)
		{
			*cfg_and_state.output << "Warning: cuBLAS setStream failed (status=" << status << ")" << std::endl;
			cublasDestroy(blasHandle[i]);
			return;
		}
		blasInit[i] = 1;
		*cfg_and_state.output << "cuBLAS initialized for device " << i << std::endl;
	}
}
...
cublasHandle_t blas_handle()
{
	...
	if (tl_cublas_handle_override) return tl_cublas_handle_override;
...
cublasLtHandle_t cublaslt_handle()
{
	...
	if (tl_cublaslt_handle_override) return tl_cublaslt_handle_override;

	int i = cuda_get_device();
	if (!cublasLtInit[i])
	{
		CHECK_CUBLAS(cublasLtCreate(&cublasLtHandle[i]));
		cublasLtInit[i] = 1;
	}
	return cublasLtHandle[i];
}
#endif

// Per-network CUDA resource accessors for thread-safe inference
cudaStream_t get_network_cuda_stream(Darknet::Network & net)
{
	TAT(TATPARMS);
	...
	if (!net.cuda_stream_initialized)
	{
		...
		if (status != cudaSuccess)
		{
			*cfg_and_state.output << "cudaStreamCreate() error: " << status << " - " << cudaGetErrorString(status) << std::endl;
			CHECK_CUDA(cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking));
		}
		net.cuda_stream = stream;
		net.cuda_stream_initialized = 1;
	}
	return (cudaStream_t)net.cuda_stream;
}

cublasHandle_t get_network_blas_handle(Darknet::Network & net)
{
	TAT(TATPARMS);
	...
	if (!net.cublas_handle_initialized)
	{
		...
		cublasStatus_t status = cublasSetStream(handle, get_network_cuda_stream(net));
		CHECK_CUBLAS(status);
		net.cublas_handle = handle;
		net.cublas_handle_initialized = 1;
	}
	return (cublasHandle_t)net.cublas_handle;
}

#ifdef DARKNET_GPU_CUDA
cublasLtHandle_t get_network_cublaslt_handle(Darknet::Network & net)
{
	TAT(TATPARMS);
	...
	if (!net.cublaslt_handle_initialized)
	{
		...
		net.cublaslt_handle = handle;
		net.cublaslt_handle_initialized = 1;
	}
	return (cublasLtHandle_t)net.cublaslt_handle;
}
#endif

#ifdef CUDNN
cudnnHandle_t get_network_cudnn_handle(Darknet::Network & net)
{
	TAT(TATPARMS);
	...
	if (!net.cudnn_handle_initialized)
	{
		...
		cudnnCreate(&handle);
		cudnnStatus_t status = cudnnSetStream(handle, get_network_cuda_stream(net));
		CHECK_CUDNN(status);
		net.cudnn_handle = handle;
		net.cudnn_handle_initialized = 1;
	}
	return (cudnnHandle_t)net.cudnn_handle;
}
#endif


/// Route all CUDA work from the calling thread to @p net's dedicated stream and cuBLAS handle.
/// Call this once at the start of any thread that will call @c network_predict on @p net.
/// Because the overrides are thread-local, concurrent threads each pointing to different networks
/// never interfere with each other.
void activate_network_streams(Darknet::Network & net)
{
	TAT(TATPARMS);

	tl_cuda_stream_override   = get_network_cuda_stream(net);
	tl_cublas_handle_override = get_network_blas_handle(net);
#ifdef DARKNET_GPU_CUDA
	tl_cublaslt_handle_override = get_network_cublaslt_handle(net);
#endif
#ifdef CUDNN
	tl_cudnn_handle_override  = get_network_cudnn_handle(net);
#endif
}

/// Clear per-thread stream overrides, reverting to the global device-indexed singletons.
/// Optional: thread_local storage is already scoped to the thread's lifetime, so this only
/// matters if the same thread later needs to use a different network (or the global stream).
void deactivate_network_streams()
{
	TAT(TATPARMS);

	tl_cuda_stream_override   = nullptr;
	tl_cublas_handle_override = nullptr;
#ifdef DARKNET_GPU_CUDA
	tl_cublaslt_handle_override = nullptr;
#endif
#ifdef CUDNN
	tl_cudnn_handle_override  = nullptr;
#endif
}
...
void show_cuda_cudnn_info()
{
	...
	if (device_count_status != cudaSuccess)
	{
		Darknet::display_warning_msg(
			"failed to query CUDA devices during startup (" +
			std::string(cudaGetErrorName(device_count_status)) + ": " +
			std::string(cudaGetErrorString(device_count_status)) + ")\n");
		*cfg_and_state.output
			<< "CUDA device enumeration is unavailable; skipping CUDA/cuDNN device details."
			<< std::endl;
		cudaGetLastError(); // clear the runtime error so command-specific setup can continue cleanly
		return;
	}

	const auto runtime_version_status = cudaRuntimeGetVersion(&cuda_runtime_version);
	if (runtime_version_status != cudaSuccess)
	{
		Darknet::display_warning_msg(
			"failed to query CUDA runtime version during startup (" +
			std::string(cudaGetErrorName(runtime_version_status)) + ": " +
			std::string(cudaGetErrorString(runtime_version_status)) + ")\n");
		*cfg_and_state.output
			<< "CUDA runtime version is unavailable; skipping CUDA/cuDNN device details."
			<< std::endl;
		cudaGetLastError();
		return;
	}

	const auto driver_version_status = cudaDriverGetVersion(&cuda_driver_version);
	if (driver_version_status != cudaSuccess)
	{
		Darknet::display_warning_msg(
			"failed to query CUDA driver version during startup (" +
			std::string(cudaGetErrorName(driver_version_status)) + ": " +
			std::string(cudaGetErrorString(driver_version_status)) + ")\n");
		*cfg_and_state.output
			<< "CUDA driver version is unavailable; skipping CUDA/cuDNN device details."
			<< std::endl;
		cudaGetLastError();
		return;
	}
	...
	*cfg_and_state.output
		...
		<< "FP16: ";
#ifndef CUDNN_HALF
	*cfg_and_state.output << Darknet::in_colour(Darknet::EColour::kBrightRed, "OFF");
#else
	*cfg_and_state.output << Darknet::in_colour(Darknet::EColour::kBrightWhite, "ON");
#endif
#endif

	// FP8 (Ada/SM89+) and FP4 (Blackwell-only) support, checked against the *current* CUDA
	// device the same way Darknet::fp8_gemm_supported() / fp4_runtime_supported() do -- these
	// calls are guarded by DARKNET_HAS_FP8/DARKNET_HAS_FP4 (set in src-lib/CMakeLists.txt only
	// when the fp8/fp4 object libraries were actually compiled in) so the message below tells
	// you whether the *build* has the feature as well as whether the *GPU* does.
	*cfg_and_state.output << ", FP8: ";
	...
	{
		*cfg_and_state.output
			<< Darknet::in_colour(Darknet::EColour::kBrightRed, "OFF")
			<< " (GPU is below SM89 / Ada)";
	}
#else
	*cfg_and_state.output << Darknet::in_colour(Darknet::EColour::kBrightRed, "OFF");
#endif

	*cfg_and_state.output << ", FP4: ";
#if defined(DARKNET_GPU_CUDA) && defined(DARKNET_HAS_FP4)
	if (Darknet::fp4_runtime_supported())
	{
		*cfg_and_state.output
			<< Darknet::in_colour(Darknet::EColour::kBrightGreen, "ON") << " -- "
			<< Darknet::in_colour(Darknet::EColour::kBrightGreen, "I HAVE BLACKWELL !!")
			<< std::endl;
	}
	else
	{
		*cfg_and_state.output
			<< Darknet::in_colour(Darknet::EColour::kBrightRed, "OFF") << " -- "
			<< Darknet::in_colour(Darknet::EColour::kBrightRed, "I DON'T HAVE BLACKWELL !! :(")
			<< std::endl;
	}
#else
	{
		// FP4 support wasn't compiled in, but we can still tell the user whether their
		// GPU *would* have qualified (Blackwell == SM100+) by querying it directly.
		bool have_blackwell = false;
		int fp4_probe_device = 0;
		cudaDeviceProp fp4_probe_prop;
		if (device_count > 0 &&
			cudaGetDevice(&fp4_probe_device) == cudaSuccess &&
			cudaGetDeviceProperties(&fp4_probe_prop, fp4_probe_device) == cudaSuccess)
		{
			have_blackwell = (fp4_probe_prop.major >= 10);
		}
		if (have_blackwell)
		{
			*cfg_and_state.output
	...
	else
	{
		...
		{
			cudaDeviceProp prop;
			const auto prop_status = cudaGetDeviceProperties(&prop, idx);
			if (prop_status != cudaSuccess)
			{
				Darknet::display_warning_msg(
					"failed to query CUDA device #" + std::to_string(idx) + " during startup (" +
					std::string(cudaGetErrorName(prop_status)) + ": " +
					std::string(cudaGetErrorString(prop_status)) + ")\n");
				cudaGetLastError();
				continue;
			}
			*cfg_and_state.output
