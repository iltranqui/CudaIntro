...
void calibrate_detector_fp8(const char *cfgfile, const char *datacfg, const char *weightfile, const int max_images);
void calibrate_detector_fp4(const char *cfgfile, const char *datacfg, const char *weightfile, const int max_images);
...
//	const std::filesystem::path & weightfile	= cfg_and_state.weights_filename;
	...
	for (int k = 0; k < ngpus; ++k)
	{
		...
		calculate_binary_weights(&nets[k]);
	...
	if (calc_map)
	{
		...
		prepare_detector_map_thread_networks(cfgfile.c_str(), &net);
	}
	...
	*cfg_and_state.output
	...
	for (int k = 0; k < net.n; ++k)
	{
		...
		if (lk.type == Darknet::ELayerType::YOLO or
			...
			lk.type == Darknet::ELayerType::CENTERNET or
			lk.type == Darknet::ELayerType::DETR_DECODER)
		{
			l = lk;
...
//			<< ", iter=" << Darknet::format_duration_string(iteration_duration, 1)
	...
	if (break_after_burn_in == false)
	{
		...
		if (net.fp8_training || net.fp4_training)
		{
			const int calibration_images = cfg_and_state.get("images", 1000000000);
			if (net.fp8_training)
			{
				calibrate_detector_fp8(cfgfile.c_str(), datacfg.c_str(), final_path.c_str(), calibration_images);
			}
			if (net.fp4_training)
			{
				calibrate_detector_fp4(cfgfile.c_str(), datacfg.c_str(), final_path.c_str(), calibration_images);
			}
		}
	...
	if (calc_map)
	{
		release_detector_map_thread_networks();
	}
...
void validate_detector(const std::string & datacfg, const std::string & cfgfile, const std::string & weightfile, const std::string & outfile)
{
	...
	*cfg_and_state.output
	...
	time_t start = time(0);
#ifdef DARKNET_GPU
	// Bind this thread's CUDA resources to the network's dedicated stream so that, if
	// validate_detector is ever called from a background thread, its kernels don't race
	// with another concurrent inference task on the default global stream.
...
void calibrate_detector_fp8(const char *cfgfile, const char *datacfg, const char *weightfile, const int max_images)
{
	...
	{
		Darknet::display_warning_msg("FP8 calibration requires GPU inference; no calibration sidecar was written.\n");
		return;
	}
	if (datacfg == nullptr || cfgfile == nullptr || weightfile == nullptr || weightfile[0] == '\0')
	{
		darknet_fatal_error(DARKNET_LOC, "detector calibrate requires data cfg, network cfg, and weights filenames");
	}

	list *options = read_data_cfg(datacfg);
	const std::string train_images = option_find_str(options, "train", "");
	const std::string valid_images = option_find_str(options, "valid", train_images);
	const std::string calibration_images = option_find_str(options, "calibrate", valid_images);
	if (calibration_images.empty())
	{
		free_list_contents_kvp(options);
		free_list(options);
		darknet_fatal_error(DARKNET_LOC, "no calibration, valid, or train image list was found in %s", datacfg);
	}
	...
	fuse_conv_batchnorm(net);
	net.fp8_inference = 0;
	net.fp8_calibrating = 1;
	calculate_binary_weights(&net);

	list *plist = get_paths(calibration_images.c_str());
	char **paths = (char **)list_to_array(plist);
	const int image_count = std::max(0, std::min(max_images, plist->size));
	if (image_count <= 0)
	{
		...
		free_list(options);
		free_network(net);
		darknet_fatal_error(DARKNET_LOC, "FP8 calibration requires at least one image");
	}
	*cfg_and_state.output << "FP8 calibration: running " << image_count << " images from " << calibration_images << std::endl;

	for (int idx = 0; idx < image_count; ++idx)
	{
		char *path = paths[idx];
		Darknet::Image orig = Darknet::load_image(path, 0, 0, net.c);
		Darknet::Image sized = net.letter_box ? Darknet::letterbox_image(orig, net.w, net.h) : Darknet::resize_image(orig, net.w, net.h);
		network_predict(net, sized.data);
		Darknet::free_image(orig);
		Darknet::free_image(sized);
		if (cfg_and_state.is_verbose && (idx + 1) % 25 == 0)
		{
			*cfg_and_state.output << "FP8 calibration: processed " << (idx + 1) << " images." << std::endl;
		}
	}

	std::vector<Darknet::Fp8CalibrationEntry> entries;
	for (int idx = 0; idx < net.n; ++idx)
	{
		Darknet::Layer & l = net.layers[idx];
		if (l.type != Darknet::ELayerType::CONVOLUTIONAL || l.fp8_amax_gpu == nullptr)
		{
			continue;
		}

		Darknet::Fp8CalibrationEntry entry;
		entry.layer_index = idx;
		entry.amax = Darknet::fp8_pull_amax_gpu(l.fp8_amax_gpu);
		entry.scale = Darknet::fp8_scale_from_amax(entry.amax);
		entries.push_back(entry);
	}

	const auto sidecar_path = Darknet::fp8_scales_sidecar_path(weightfile);
	if (entries.empty() || !Darknet::fp8_write_calibration_scales(sidecar_path, entries))
	{
		free_ptrs((void **)paths, plist->size);
		free_list(plist);
		free_list_contents_kvp(options);
		free_list(options);
		free_network(net);
		darknet_fatal_error(DARKNET_LOC, "failed to write FP8 calibration sidecar %s", sidecar_path.string().c_str());
	}

	*cfg_and_state.output << "FP8 calibration: wrote " << entries.size() << " layer scales to " << sidecar_path << std::endl;
	free_ptrs((void **)paths, plist->size);
	free_list(plist);
	free_list_contents_kvp(options);
	free_list(options);
	free_network(net);
#else
	Darknet::display_warning_msg("FP8 calibration requires an Ada-or-newer CUDA build with FP8 support; no calibration sidecar was written.\n");
	return;
#endif
}
...
void calibrate_detector_fp4(const char *cfgfile, const char *datacfg, const char *weightfile, const int max_images)
{
	...
	if (cfg_and_state.gpu_index < 0)
	{
		Darknet::display_warning_msg("FP4 calibration requires GPU inference; no calibration sidecar was written.\n");
		return;
	}
	if (datacfg == nullptr || cfgfile == nullptr || weightfile == nullptr || weightfile[0] == '\0')
	{
	...
	if (image_count <= 0)
	{
		...
		free_list(options);
		free_network(net);
		darknet_fatal_error(DARKNET_LOC, "FP4 calibration requires at least one image");
	}
	*cfg_and_state.output << "FP4 calibration: running " << image_count << " images from " << calibration_images << std::endl;
	...
	for (int idx = 0; idx < image_count; ++idx)
	{
		...
		if (cfg_and_state.is_verbose && (idx + 1) % 25 == 0)
		{
			*cfg_and_state.output << "FP4 calibration: processed " << (idx + 1) << " images." << std::endl;
		}
	}

	std::vector<Darknet::Fp4CalibrationEntry> entries;
	for (int idx = 0; idx < net.n; ++idx)
	{
		Darknet::Layer & l = net.layers[idx];
		if (l.type != Darknet::ELayerType::CONVOLUTIONAL || l.fp4_amax_gpu == nullptr)
		{
			continue;
		}

		Darknet::Fp4CalibrationEntry entry;
		entry.layer_index = idx;
		entry.amax = Darknet::fp4_pull_amax_gpu(l.fp4_amax_gpu);
		entry.scale = Darknet::fp4_scale_from_amax(entry.amax);
		entries.push_back(entry);
	}

	const auto sidecar_path = Darknet::fp4_scales_sidecar_path(weightfile);
	if (entries.empty() || !Darknet::fp4_write_calibration_scales(sidecar_path, entries))
	{
		free_ptrs((void **)paths, plist->size);
		free_list(plist);
		free_list_contents_kvp(options);
		free_list(options);
		free_network(net);
		darknet_fatal_error(DARKNET_LOC, "failed to write FP4 calibration sidecar %s", sidecar_path.string().c_str());
	}

	*cfg_and_state.output << "FP4 calibration: wrote " << entries.size() << " layer scales to " << sidecar_path << std::endl;
	Darknet::display_warning_msg("Note: the .fp4scales sidecar is currently a diagnostic/audit artifact -- the FP4 GEMM's "
		"cuDNN Frontend block-scale-quantize op computes its scales internally per call with no exposed override, "
		"so this calibration does not yet change FP4 inference numerics.\n");
	free_ptrs((void **)paths, plist->size);
	free_list(plist);
	free_list_contents_kvp(options);
	free_list(options);
	free_network(net);
#else
	Darknet::display_warning_msg("FP4 calibration requires a Blackwell CUDA build with FP4 support; no calibration sidecar was written.\n");
	return;
#endif
}
...
//	int frame_skip = find_int_arg(argc, argv, "-s", 0);
	...
	else if (cfg_and_state.function == "calibrate"	)
	{
		const int calibration_images = cfg_and_state.get("images", 1000000000);
		if (cfg_and_state.is_set("fp4"))
		{
			calibrate_detector_fp4(cfg, datacfg, weights, calibration_images);
		}
		else
		{
			calibrate_detector_fp8(cfg, datacfg, weights, calibration_images);
		}
	}
