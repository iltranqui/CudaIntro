...
const Darknet::SArgsAndParms & Darknet::get_all_possible_arguments()
{
	...
	static const SArgsAndParms all =
	{
		...
		ArgsAndParms("calibrate"	, ArgsAndParms::EType::kFunction, "Calibrate FP8 activation scales for inference."),
		...
		ArgsAndParms("precisionbenchmark", ArgsAndParms::EType::kCommand, "Benchmark synthetic device-only inference with CUDA events."),
		...
		ArgsAndParms("bf16"			, ArgsAndParms::EType::kParameter	, "Use cuDNN BF16 Tensor Core convolutions when supported by the GPU."),
		ArgsAndParms("fp4"			, ArgsAndParms::EType::kParameter	, "Use Blackwell NVFP4 block-scaled Tensor Core GEMMs when supported by CUDA 13.2 and the installed cuDNN Frontend's block-scale API."),
		ArgsAndParms("fp8"			, ArgsAndParms::EType::kParameter	, "Use FP8 convolutions when supported by the GPU:  FP8 training GEMMs when training, calibrated FP8 E4M3 inference otherwise."),
		ArgsAndParms("tf32"			, ArgsAndParms::EType::kParameter	, "Use TF32 tensor-core math with FP32 data (Ampere or newer GPU).  Disables the FP16, BF16, and FP8 paths."),
		...
		ArgsAndParms("precisionbatch", "", 4, "Batch size for precision-benchmark.  -precision_batch 4"),
		ArgsAndParms("precisionwarmup", "", 30, "Warmup iterations for precision-benchmark.  -precision_warmup 30"),
		ArgsAndParms("precisioniterations", "", 200, "Timed iterations for precision-benchmark.  -precision_iterations 200"),
		...
		ArgsAndParms("images"				, "", 1000000000, "The number of images to use for calibration-oriented commands (default: every image in the list)."),
...
void Darknet::display_usage()
{
	...
	*cfg_and_state.output
		...
		<< ""																						<< std::endl
		<< "  Train a network using FP8 (Ada or newer GPU required):"								<< std::endl
		<< YELLOW("    darknet detector train -map -dont_show -fp8 cars.data cars.cfg")				<< std::endl
		<< "  FP8 activation scales are calibrated automatically on cars_best_final.weights at the"	<< std::endl
		<< "  end of training, writing \"..._final.weights.fp8scales\" next to it. To (re-)calibrate"	<< std::endl
		<< "  by hand -- every image in the list is used by default, override with -images:"			<< std::endl
		<< YELLOW("    darknet detector calibrate cars.data cars.cfg cars_best.weights")			<< std::endl
		<< YELLOW("    darknet detector calibrate cars.data cars.cfg cars_best.weights -images 512") << std::endl
		<< "  Run inference in FP8 using the calibrated weights (the .fp8scales sidecar must exist):" << std::endl
		<< YELLOW("    darknet detector test -fp8 cars.data cars.cfg cars_best.weights image1.jpg") << std::endl
		<< ""																						<< std::endl
		<< "  Train a network using FP4 (Blackwell GPU required):"								<< std::endl
		<< YELLOW("    darknet detector train -map -dont_show -fp4 cars.data cars.cfg")				<< std::endl
		<< "  FP4 per-layer activation ranges are likewise calibrated automatically at the end of"	<< std::endl
		<< "  training (diagnostic -- FP4's block-scale quantization is computed on the fly and"		<< std::endl
		<< "  does not yet consume this). To (re-)calibrate by hand, same -images override:"			<< std::endl
		<< YELLOW("    darknet detector calibrate -fp4 cars.data cars.cfg cars_best.weights")		<< std::endl
		<< YELLOW("    darknet detector calibrate -fp4 cars.data cars.cfg cars_best.weights -images 512") << std::endl
		<< ""																						<< std::endl;
