...
//	std::cout << "load_rgb_mat_image(" << filename << ")" << std::endl;
	...
	{
		// e.g. RGB + motion channel packed in the alpha plane
	...
	if (channels == 4 and mat.channels() != 4)
	{
		darknet_fatal_error(DARKNET_LOC, "expected a 4-channel image but \"%s\" has %d channels", filename.c_str(), mat.channels());
	}
	...
	int gaussian_noise, int blur, int fog, int num_boxes, int truth_size, float *truth)
{
	...
	try
	{
		...
		if (dsat != 1 || dexp != 1 || dhue != 0)
		{
			...
			{
				// photometric augmentation applies to the RGB planes only -- the 4th
				// plane carries a motion/gradient signal which must not be jittered
				...
				cv::Mat hsv_src;
				cvtColor(rgb, hsv_src, cv::COLOR_RGB2HSV);
				...
				cv::merge(hsv, hsv_src);
				cvtColor(hsv_src, rgb, cv::COLOR_HSV2RGB);

				std::vector<cv::Mat> rgb_planes;
				cv::split(rgb, rgb_planes);
				cv::merge(std::vector<cv::Mat>{rgb_planes[0], rgb_planes[1], rgb_planes[2], planes[3]}, sized);
			}
			else
			{
				sized *= dexp;
		...
		if (fog)
		{
			...
			// 4th value only used for 4-channel inputs: no fog on the motion plane
			fog_mask.setTo(cv::Scalar(180, 178, 166, 0));
			...
			if (sized.channels() == 4)
			{
				// fog must not fade the motion plane -- restore it after blending
				std::vector<cv::Mat> planes;
				cv::split(sized, planes);
				cv::Mat motion = planes[3].clone();
				cv::addWeighted(sized, t, fog_mask, 1 - t, 0.0, sized);
				cv::split(sized, planes);
				motion.copyTo(planes[3]);
				cv::merge(planes, sized);
			}
			else
			{
				cv::addWeighted(sized, t, fog_mask, 1 - t, 0.0, sized);
		...
		if (sized.channels() == 4)
		{
			// rgb_mat_to_rgb_image() only handles 3 planes; mat_to_image() handles any
